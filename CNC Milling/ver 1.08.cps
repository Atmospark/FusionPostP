...
09-04-2026		-	Added ability to skip first tool change in post settings
						Added ability to pause between operations by simulating a tool change to a nonexistent tool
...
obversion = ' 1.08';    								// date updated 13-04-2026
...
groupDefinitions = {
	...
	jobConstraints: {title:"Job Constraints", description: "Options relevant to job which will affect your machine or Commander behaviour", collapsed:false, order:20}
...
properties =
	...
	opHold:{
		title		: "Pause Between Operations",
		description	: "Forces Commander to pause between operations by simulating a tool change to a nonexistent tool. Must have 'Output Tool Change Data' ticked",
	...
	opHoldTool:{
		title		: "Operation Hold Tool",
		description	: "Specify nonexistent tool used to pause job between operations.",
		group		: "jobConstraints",
		type		: "number",
		value		: 100,
		scope		: "post"
	...
	ignoreFirstTool: {
		title		: "Skip Initial Tool",
		description	: "Skips adding initial tool change command into GCODE so Commander ignores first tool change",
		group		: "jobConstraints",
		type		: "boolean",
		value		: false,
		scope		: "post"
    ...
    spindleTwoDirections: {
	 ...
	 toolOutput: {
	  ...
	  debugComment: {																							// debug output not needed in production release
...
function writeComment(text)
   ...
   if (text.length > 140)
      ...
      for (i = 0; i < bits.length; i++)
         ...
         if (out.length > 1300)           // a long word on the end can take us to 80 chars!
            {
...
function onOpen()
	...
	writeComment("G-Code optimised for " + myMachine.getModel() + " controller using " + myMachine.getControl() );
	...
	for (var i = 0; i < numberOfSections; ++i)								// insert tool data at top of GCODE formatted specifically for Commander
		{		         
		...
		var opHoldTool = getProperty("opHoldTool");

	if (getProperty("opHold") == true)
		{
			writeTool((tFormat.format(opHoldTool)) + ": " + "User-requested Job Hold");
		}
	...
	for (var i = 0; i < numberOfSections; ++i)
	    ...
	    //add tool info as a comment with tool number linked to section number. Multiple tools in same job will force error in post so this works for Commander as intended GCODE sender to read tool info and display to user
   		...
   		if (getProperty("toolOutput") == true && (getProperty("opHold") == true))
		 	{
			writeComment("Tool" + " " + tooli.number + ": " + toTitleCase(getToolTypeName(tool.type)) + " " + tool.numberOfFlutes + " Flute" + ((tool.numberOfFlutes == 1)?"":"s") + ", Diam = " + tool.diameter + toolUnit + ", Len = " + tool.fluteLength + toolUnit + ", RPM = " + tool.spindleRPM + " ");
		 	writeComment("Tool" + " " + opHoldTool + ": " + "User-Requested Job Hold");
			}
		

		else if (getProperty("toolOutput") == true)
		 	{
			writeComment("Tool" + " " + tooli.number + ": " + toTitleCase(getToolTypeName(tool.type)) + " " + tool.numberOfFlutes + " Flute" + ((tool.numberOfFlutes == 1)?"":"s") + ", Diam = " + tool.diameter + toolUnit + ", Len = " + tool.fluteLength + toolUnit + ", RPM = " + tool.spindleRPM + " ");
			}
...
function onSection()
	...
	//var tChange = false;									// Set spindle status default true
	//var rpmChange;
	//var spinStop ;
	...
	if ((section.workOffset < 1) || (section.workOffset > 6))
	 ...
	 if(isFirstSection() && (getProperty("toolOutput") == true) && (getProperty("ignoreFirstTool") == false))				//skips initial tool change command if requested by user
	 	{
		...
		if ((i > 0) && (i < getNumberOfSections()))								// Add dwell only if rpm/tool changes between operations
			{
				var pSec = i-1;
				var pSecNum = i;
				var pSection = getSection(i-1);
				...
				if ((tool.spindleRPM != pRPM)  )
						{
							rpmChange = true;
							spinStop = true;
						}
				
				else if ((pTool.number != tool.number))
						{
							tChange = true;
							spinStop = true;
						}		
				if (rpmChange != tChange) 
						{
							spinStop = true;
						}	
					...
					if (getProperty("debugComment") == true)							// debug output
					{
						writeln("");
						writeComment("prev. section number: " + " " + pSecNum + " " + "/" + pSec + "/" ); 				
						writeComment("curr. section number: " + " " + cSecNum + " " + "/" + cSec + "/" ); 			
						writeComment("previous RPM: "+ " " + pRPM);							
						writeComment("current RPM: " + " " + tool.spindleRPM);							
						writeComment("Prev. Tool: " + " " + pTool.number); 	
						writeComment("Curr. Tool: " + " " + tool.number); 	
						writeComment("Tool Changed?:" + " " + tChange);		
						writeComment("Speed Changed?:" + " " + rpmChange);			
						writeComment("Spindle Stopped?:" + " " + spinStop);	
						writeComment("Operation Hold?:" + " " + getProperty("opHold"));
						writeln("");
					}	
			...
			if (spinStop == true || isFirstSection() || getProperty("opHold") == true)
				{
...
/*			var i = getCurrentSectionId();
	...
	//			var cTool = cSection.getTool();
	//			var cRPM = cTool.spindleRPM;
	//			var cTool = tool;
...
/*					if ((tool.spindleRPM == pRPM) )
					...
					if ((pTool.number == tool.number)) 
						{
							tChange = false;
						}	
					
	/*				else if ((cRPM != pRPM) || (pTool.number != cTool.number)) 
						{	
							spinStop = true;
						}
*/
	...
	if ((isFirstSection()) || spinStop == true || getProperty("opHold") == true) 							// Wait some time for spindle to speed up on first section or if spindle speed changes between sections or if tool changes
		{
		...
		if (getCurrentSectionId() <  getNumberOfSections()-1  && (getProperty("toolOutput") == true))
			{
				...
				var opHoldTool = getProperty("opHoldTool");
					...
					if ( (tooli.spindleRPM != toolj.spindleRPM))
						{
					...
					else if ((tooli.number != toolj.number))
						{
							writeBlock(mFormat.format(5));
							onDwell(getProperty("spindleDwell"));	
							writeBlock(tFormat.format(toolj.number));
					...
					else if ((tooli.number = toolj.number) && getProperty("opHold") == true)
						{
							writeBlock(mFormat.format(5));
							onDwell(getProperty("spindleDwell"));
							writeBlock(tFormat.format(opHoldTool));
